...
<div id="form" class = "well span12 noleftmargin">
	...
	<div id="alertMsg" style="display:none" class="alert alert-error">
		...
		<div class="manualrating">
			...
			<div class="span4">
				...
				echo $this->Form->input('no_limit_hidden', array(
						'type'		=> 'text',
						'value'	=> ($row['Limit']['max_concentration_abs'] == ''),
						'style'	=> 'display:none;',
						'label' => false,
						'div'	=> false,
					)
		...
		<div class="span11">
			...
			?>
			<?php if(!empty($id)): ?>
				<a class="btn btn-danger pull-right btn-small" style="margin-right: 5px;" id="delete_limit">
				<i class="icon-trash icon-white" data-id-limit="<?php echo $id; ?>"></i>
				Remove</a>
			<?php endif ?>
...
<?php
echo $this->Form->create('automaticValue', array('url'=>'/treasury/treasurystaticdatas/limitGetCalculated'));
echo $this->Form->input('limit.mandategroupId', array(
	'type' => 'text',
	'label'	=> false,
	'div'	=> false,
));
echo $this->Form->input('limit.cpty', array(
	'type' => 'text',
	'label'	=> false,
	'div'	=> false,
));
...
echo $this->Form->create('nolimit', array('url'=>'/treasury/treasuryajax/getNoLimitOnMandateGroup'));
echo $this->Form->input('limit.mandate_id', array(
	'type' => 'text',
	'label'	=> false,
	'div'	=> false,
));
echo $this->Form->end();
echo $this->Form->create('dellimit', array('url'=>'/treasury/treasurystaticdatas/limite_delete'));
echo $this->Form->input('limit.limit_id', array(
	'type' => 'text',
	'label'	=> false,
	'div'	=> false,
));
...
<script>
	...
	$(document).ready(function(){
		...
		$('#delete_limit').click(function(e)
		{
			if (confirm("are you sure you want to delete this entry?"))
			{
				var id = $(e.target).find('i').attr('data-id-limit');
				$("#dellimitLimitForm #limitLimitId").val( id );
				$("#dellimitLimitForm").submit();
			}
		});
		...
		$('#LimitLimitEur, #LimitMaxConcentrationAbs').autoNumeric('init',{aSep: ',',aDec: '.', vMin:0, mDec:2, vMax: 9999999999999.99, vMin:0.00});

	});
	
	function send_value()
	{
		$('#LimitNoLimit').prop('readonly', false);
	...
	function manualAutomaticToggle(e, noval){
		if($('#LimitAutomatic').val()==1){
			...
			$('#LimitNoLimit').prop("readonly", true);
		}else{
			$('.manualrating input').prop('readonly', false);
			$('.manualrating input').not('.optional').prop('required', true);
			$('#LimitNoLimit').prop("readonly", false);//http://vmu-sas-01:8080/browse/TREASURY-388
		}
	}

	function getAutomaticValues(e){
		//ajax to get automatic values
		var url = '/treasury/treasurystaticdatas/limitGetCalculated';
		$('#automaticValueLimitForm #limitMandategroupId').val($('#LimitLimitForm #LimitMandategroupID').val());
		$('#automaticValueLimitForm #limitCpty').val($('#LimitLimitForm #LimitCptyID').val());
		var data = $('#automaticValueLimitForm').serialize();
		automaticValues = {};
		$.ajax({
			type: "POST",
			url: url,
	...
	function init_no_limit()
	{
		if (is_not_limit_automaticMode)
		{
			...
			$('#LimitNoLimitHidden').val('1');
			$('#LimitMaxConcentrationAbs').attr('readonly', true);
			$('#LimitMaxConcentration').attr('readonly', true);
		}
		else		
		{
			$('#LimitNoLimit').prop('checked', false);
			$('#LimitNoLimitHidden').val('0');
			$('#LimitMaxConcentrationAbs').attr('readonly', false);
			$('#LimitMaxConcentration').attr('readonly', false);
		}
	}
	...
	{
		$('#nolimitLimitForm #limitMandateId').val( $('#LimitLimitForm #LimitMandategroupID').val() );
		var data = $('#nolimitLimitForm').serialize();
		var url = '/treasury/treasuryajax/getNoLimitOnMandateGroup';
		$.ajax({
			type: "POST",
			url: url,
			dataType: 'text', 
