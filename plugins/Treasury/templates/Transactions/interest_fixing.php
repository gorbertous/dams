...
<fieldset>
	...
	<?php
		...
		echo $this->Form->submit('Automatic fixing', array('div' => false));
	?>
<?php echo $this->Form->end(); ?>
   ...
   <?php echo $this->Form->input('cpty_id', array(
    ...
    <?php if (sizeof($transactions) > 0): ?>
        ...
        <?php echo $this->Form->create('intfixing', array('name'=>'fixingform', 'url' => '/treasury/treasurytransactions/interest_fixing'));
        ...
        <table id="selectCall" class="table table-bordered table-striped table-hover table-condensed">
            ...
            <?php foreach ($transactions as $transaction): ?>
                <tr>
                    <td>
					<input type="radio" class="origin_radio" name="data[Transaction][tr_number]" value="<?php echo $transaction['Transaction']['tr_number'] ?>" id="data[Transaction][tr_number]"/>
                        
						<?php
						
						$date  = new DateTime(str_replace('/','-',$transaction['Transaction']['commencement_date']));
						echo $this->Form->input('com_date', array(
							'type' => 'hidden',
							'label'	=> false,
							'div'	=> false,
							'class'	=> 'com_date',
                        ...
                        if (($transaction['Transaction']['maturity_date'] != null) && ($transaction['Transaction']['tr_state'] == 'Called'))
                        {
							...
							echo $this->Form->input('mat_date', array(
								'type' => 'hidden',
								'label'	=> false,
								'div'	=> false,
        ...
        <script>
            ...
            $(document).ready(function(){
				...
				function ajax_submit()
				{
					...
					$('#fixInterestajaxInterestFixingForm #TransactionTrNum').val( $('.selected').find('#intfixingTrNumber').val() );
...
<?php
		...
		if (isset($success))
		{
			echo "document.location = '".$success."';";
		}
			?>
