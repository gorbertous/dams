...
?>
	...
	<div class="span11 noleftmargin">
		...
		<div class="span5 text-right">
			<?php echo $this->Html->link("DI: <i class='icon-file'></i>", "/treasury/treasuryajax/download_file/1?file=/data/treasury/pdf/deposit_instruction_".$instr['Instruction']['instr_num'].".pdf", array('escape' => false, 'target' => '_blank')) ?>&nbsp;    
...
<?php else: ?>
		...
		<?php echo $this->Html->link("Confirmation <i class='icon-file'></i>", "/treasury/treasuryajax/download_file/1?file=/data/treasury/pdf/".$instr['Instruction']['confirmation_file'], array('escape' => false, 'target' => '_blank')) ?>
<?php endif ?>
			</div>
		</div>
				...
				<?php foreach ($trns as $trnum => $trn): ?>
					<?php 
						...
						if(isset($trn['reference_rate']) && isset($trn['interest_rate'])){
							$spread = (floatval($trn['interest_rate'])-floatval($trn['reference_rate']))*100;
						}
					...
					<tr class="formQueueLine" id="rowTrn<?php echo $trnum ?>" data-trnum="<?php echo $trnum ?>" data-date-com="<?php print $date_com->format('j-n-Y') ?>" data-date-mat="<?php print $date_mat->format('d-m-Y') ?>" data-currency="<?php echo $trn['currency'] ?>" >
						...
						<td class="control-group external_ref optional">
							<?php
							echo $this->Form->input('Transaction.external_ref', array(
								'type' => 'text',
								'label'	=> false,
								'div'	=> false,
						...
						<td class="control-group interest_rate">
								<?php	echo $this->Form->input(
									'Transaction.interest_rate', array(
										'label'     => false,
										'id'	=> false,
										'div'	=> false,
										//'name'	=> 'data[Transaction][interest_rate]',
										'class' => 'interest_rate TransactionInterestRate',
						...
						<td class="control-group date_basis">
							<?php
								$options_date_basis = array(
										"Act/360"	=>	"Act/360",
										"Act/365"	=>	"Act/365",
										"30/360"	=>	"30E/360"
									);
								if ($trn['cpty_id'] == 65)// BGL BNP PARIBAS
								{
									$options_date_basis = array(
										"Act/365"	=>	"Act/365",
									);
								}
								elseif($trn['cpty_id'] == 2)//Banque et Caisse d'Epargne de l'Etat
								{
									$options_date_basis = array(
										"Act/360"	=>	"Act/360",
									);
...
<?php
echo $this->Form->create('register', array('url'=>'/treasury/treasurytransactions/registerInstrConf'));
echo $this->Form->input('Transaction.commencement_date', array(
	'type' => 'text',
	'label'	=> false,
	'div'	=> false,
...
echo $this->Form->input('Transaction.instr_num', array(
	'type' => 'hidden',
	'label'	=> false,
	'div'	=> false,
	'value'	=> $instr['Instruction']['instr_num'],
...
$(document).ready(function(e){
	...
	//number format
	$('.TransactionInterestRate, .TransactionRefRate').autoNumeric('init',{aSep: ',',aDec: '.', vMin:-9999999999999.999,vMax: 9999999999999.99});
	$('.TransactionTaxAmount, .TransactionTotalInterest').autoNumeric('init',{aSep: ',',aDec: '.', vMin:-9999999999999.99,vMax: 9999999999999.99});
	...
	$('.refreshTaxAmount').bind('click', function(e){
		...
		$.ajax({context: {line: line}, data: datas, url: ajaxpath+'/calcTotalInterest?callback=?'})
		.done(function( data ){
			...
			if(data && data!='.' && data!='-' && data.length<20){
				...
				$.ajax({context: {line: this.line}, data: datas, url: ajaxpath+'/computeTaxFromInterestAndMandateCpty?callback=?'})
				.done(function( data ){
					if(!data) data='0.00';
	...
	function formQueueNext(){
		if(formQueue.length){
			...
			$("#registerRegisterInstrConfForm #TransactionAmount").val( currentStepDatas['data[Transaction][amount]'] );
			$("#registerRegisterInstrConfForm #TransactionCommencementDate").val( currentStepDatas['data[Transaction][commencement_date]'] );
			$("#registerRegisterInstrConfForm #TransactionCptyId").val( currentStepDatas['data[Transaction][cpty_id]'] );
			$("#registerRegisterInstrConfForm #TransactionDateBasis").val( currentStepDatas['data[Transaction][date_basis]'] );
			$("#registerRegisterInstrConfForm #TransactionDepoTerm").val( currentStepDatas['data[Transaction][depo_term]'] );
			$("#registerRegisterInstrConfForm #TransactionDepoType").val( currentStepDatas['data[Transaction][depo_type]'] );
			$("#registerRegisterInstrConfForm #TransactionExternalRef").val( currentStepDatas['data[Transaction][external_ref]'] );
			$("#registerRegisterInstrConfForm #TransactionInterestRate").val( currentStepDatas['data[Transaction][interest_rate]'] );
			$("#registerRegisterInstrConfForm #TransactionMandateID").val( currentStepDatas['data[Transaction][mandate_ID]'] );
			$("#registerRegisterInstrConfForm #TransactionMaturityDate").val( currentStepDatas['data[Transaction][maturity_date]'] );
			$("#registerRegisterInstrConfForm #TransactionReferenceRate").val( currentStepDatas['data[Transaction][reference_rate]'] );
			$("#registerRegisterInstrConfForm #TransactionScheme").val( currentStepDatas['data[Transaction][scheme]'] );
			$("#registerRegisterInstrConfForm #TransactionTaxAmount").val( currentStepDatas['data[Transaction][tax_amount]'] );
			$("#registerRegisterInstrConfForm #TransactionTotalInterest").val( currentStepDatas['data[Transaction][total_interest]'] );
			$("#registerRegisterInstrConfForm #TransactionTrNumber").val( currentStepDatas['data[Transaction][tr_number]'] );
			$("#registerRegisterInstrConfForm #TransactionTrState").val( currentStepDatas['data[Transaction][tr_state]'] );
			$("#registerRegisterInstrConfForm #TransactionInstr_num").val( currentStepDatas['data[Transaction][instr_num]'] );
			var data = $("#registerRegisterInstrConfForm").serialize();
			$.ajax({
			  'url': '/treasury/treasurytransactions/registerInstrConf',
			  type: "POST",
			  data: data,
			}).done(function( data ){
