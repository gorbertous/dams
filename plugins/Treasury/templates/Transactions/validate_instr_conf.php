				...
				<?php foreach ($trns as $trnum => $trn): ?>
					...
					<tr class="formQueueLine" id="rowTrn<?php echo $trnum ?>" data-trnum="<?php echo $trnum ?>" data-date-com="<?php print $date_com->format('j-n-Y') ?>" data-date-mat="<?php print $date_mat->format('d-m-Y') ?>" data-currency="<?php echo $trn['currency'] ?>" >
						...
						<td class="control-group">
							<?php
								echo $this->Form->input('tr_number', array(
										'type' => 'hidden',
										'label'	=> false,
										'div'	=> false,
							...
							<div class="btn-group" data-toggle="buttons-checkbox">
								<a href="#" class="btn btn-danger register-btn formQueueLaunchOne" data-tr-number='<?php echo $trnum;?>' data-di-number='<?php $instr['Instruction']['instr_num']; ?>' data-action="reject">Reject</a>
...
<?php
echo $this->Form->create('conf', array('url'=>'/treasury/treasuryajax/confvalidate/0/1'));
echo $this->Form->input('tr_number', array(
	'type' => 'text',
	'label'	=> false,
	'div'	=> false,
...
echo $this->Form->create('rej', array('url'=>'/treasury/treasuryajax/confreject/0/1'));
echo $this->Form->input('tr_number', array(
	'type' => 'text',
	'label'	=> false,
	'div'	=> false,
...
$(document).ready(function(e){
	...
	$('.formQueueLaunchOne').bind('click', function(e){
		...
		if (! confirm('Reject DI:'+DI+' with transaction '+TRN+'?'))
		{return false;}
	...
	$('.formQueueLaunchAll').bind('click', function(e){
		formQueue = [];
		if (! confirm('Are you sure you want to reject all confirmations?'))
		{return false;}
		var bt = this;
	...
	function formQueueNext(){
		if(formQueue.length){
			...
			currentStepDatas.tr_number = currentStepDatas['data[tr_number]'];
			var data = {};
			if(currentStepDatas.tr_number)
			{
				if(currentStepDatas.action=='validate')
				{
					$('#confValidateInstrConfForm #confTrNumber').val( currentStepDatas.tr_number );
					data = $('#confValidateInstrConfForm').serialize();
					currentStepDatas.url = urlbasepath+'/confvalidate/0/1';
				}
				else if(currentStepDatas.action=='reject')
				{
					$('#rejValidateInstrConfForm #rejTrNumber').val( currentStepDatas.tr_number );
					data = $('#rejValidateInstrConfForm').serialize();
					currentStepDatas.url = urlbasepath+'/confreject/0/1';
				}
			}
