...
<fieldset>
    <legend> Delete Reinvestment </legend>
    <?php if (sizeof($deletableReinvs) > 0): ?>
       
        <?php $this->BootstrapTables->displayRawsAndLinks('selectReinvestment', $deletableReinvs, array('reinvestments'), array('delete'), array('Delete'), array('btn-danger'), array('icon-remove'), 'reinv_group', true); ?>
    ...
    <?php else: ?>
    <div class="well">There are no reinvestments which can be deleted.</div>
    <?php endif ?>
