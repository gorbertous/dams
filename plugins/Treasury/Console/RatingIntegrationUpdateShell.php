...
class RatingUpdateShell extends Shell
{
	...
	public function main($debug = false)
	{
		...
		$url = "/sftp/eif_user/home/IN/counterparties.json";
					...
					if (!empty($new['NUM_TIERS'])) {
						...
						if (empty($existing) || !empty($existing['Rating']['automatic'])) {
							...
							if (!empty($new['ST_RATING_DATE']) && !is_array($new['ST_RATING_DATE'])) {
								$up['Rating']['ST-EIB_date'] = $this->dbdate($new['ST_RATING_DATE']);
								$updatedRating = true;
							}

							//							if(!empty($new['EU_CENTRAL_BANK']) && !is_array($new['EU_CENTRAL_BANK'])){ $up['Rating']['EU_CENTRAL_BANK'] = $this->dbdate($new['ST_RATING_DATE']); $updatedRating = true; }
