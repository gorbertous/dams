 ...
 * Users Model
 *
 * @property \UserMgmt\Model\Table\ProfilesTable&\Cake\ORM\Association\BelongsToMany $Profiles
 * 
...
class UsersTable extends Table
{
    ...
    public function initialize(array $config): void
    {
        ...
        // ]);
        $this->belongsToMany('Profiles', ['className'=>'UserMgmt\Model\Table\ProfilesTable', 
    ...
    public static function getEffectivePermissions(int $id) : Array {
        $user = (new UsersTable())->get($id, ['contain' => ['Profiles']]);
        $effective = [];
        foreach($user->profiles as $profile) {
            $permissions = (new PermissionsTable())->find()->select(['plugin','controller','action','allowed_int' => 'cast(allowed as signed)'])->where(['user_group_id' => $profile->get('id')])->all()->toArray();//
            foreach($permissions as $perm) {
                $action = explode('/', $perm['action'], 2);
                if (isset($effective[$perm['plugin']][$perm['controller']][$action[0]])) {
                    $effective[$perm['plugin']][$perm['controller']][$action[0]]['value']|=(int)$perm['allowed_int'];
                } else {
                    $effective[$perm['plugin']][$perm['controller']][$action[0]]['value']=(int)$perm['allowed_int'];
                }
                if (count($action)<2) {
                ...
                if (isset($effective[$perm['plugin']][$perm['controller']][$action[0]][$action[1]])) {
                    $effective[$perm['plugin']][$perm['controller']][$action[0]][$action[1]]|=(int)$perm['allowed_int'];
                } else {
                    $effective[$perm['plugin']][$perm['controller']][$action[0]][$action[1]]=(int)$perm['allowed_int'];
                }
            }
